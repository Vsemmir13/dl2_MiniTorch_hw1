...
def test_sigmoid(a: float) -> None:
    ...
    * It is  strictly increasing.
    ...
    assert lt(sigmoid(a), sigmoid(a + 1))
...
def test_transitive(a: float, b: float, c: float) -> None:
    ...
    assert (not lt(a, b)) or (not lt(b, c)) or lt(a, c)
...
def test_other(a: float, b: float, c: float) -> None:
    ...
    assert_close(a + b + c, add(a, add(b, c)))
...
def test_sum_distribute(ls1: List[float], ls2: List[float]) -> None:
    ...
    """
    list_result = addLists(ls1, ls2)
    for i, j, res in zip(ls1, ls2, list_result):
        assert_close(add(i, j), res)
